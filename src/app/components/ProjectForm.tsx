...
const ISO_REGIONS = ["PJM", "MISO", "SPP", "CAISO", "NYISO", "ISO-NE", "ERCOT", "SERC"];
...
const PROJECT_MANAGERS = ["Sarah Chen", "Mike Torres", "Emily Johnson", "James Wright", "Lisa Park", "Carlos Mendez", "David Kim", "Anna Rivera", "Robert Williams", "Patricia Lee", "Tom Anderson", "Jennifer White"];

const ISO_PER_MW_RATES: Record<string, number> = {
  PJM: 2000,
  MISO: 4000,
  SPP: 2000,
  CAISO: 10000,
...
function InputField({ label, name, type = "text", value, onChange, required, placeholder, error }: {
  label: string; name: string; type?: string; value: string | number; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean; placeholder?: string; error?: string;
}) {
  return (
    <div>
...
function SelectField({ label, name, value, onChange, options, required, error }: {
  label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; options: string[]; required?: boolean; error?: string;
}) {
  return (
    <div>
      <label htmlFor={name} className="block text-xs font-medium text-slate-500 mb-1">{label}{required && " *"}</label>
...
function TextAreaField({ label, name, value, onChange, rows = 3, error }: {
  label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void; rows?: number; error?: string;
}) {
  return (
...
export default function ProjectForm({ initialData, isEdit }: ProjectFormProps) {
  ...
  const defaults: Record<string, unknown> = {
    ...
    phase3_study_status: "Not Started",
    phase3_study_completion_date: "",
    ia_execution_date: "",
  ...
  // Auto-populate next_security_milestone when ISO or size_mw changes
  const autoPopulateSecurity = useCallback((updatedForm: Record<string, unknown>) => {
    const iso = updatedForm.iso_region as string;
    const sizeMw = Number(updatedForm.size_mw) || 0;
    const currentMilestone = Number(updatedForm.next_security_milestone) || 0;
    if (currentMilestone === 0 && sizeMw > 0 && ISO_PER_MW_RATES[iso]) {
      return { ...updatedForm, next_security_milestone: sizeMw * ISO_PER_MW_RATES[iso] };
    }
    return updatedForm;
  }, []);

  const studyStatusToDateMap: Record<string, string> = {
    feasibility_study_status: "feasibility_study_completion_date",
    system_impact_study_status: "system_impact_study_completion_date",
    facilities_study_status: "facilities_study_completion_date",
    phase3_study_status: "phase3_study_completion_date",
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setForm((prev) => {
      ...
      // Auto-set completion date when study status changes to "Completed"
      if (name.endsWith("_study_status") && value === "Completed") {
        const dateField = studyStatusToDateMap[name];
        if (dateField && !updated[dateField]) {
          updated[dateField] = getTodayString();
        }
      }

      // Status dependency logic for interconnection_status
      if (name === "interconnection_status") {
        const today = getTodayString();

        if (value === "System Impact Study") {
          if (updated.feasibility_study_status !== "Completed") {
            updated.feasibility_study_status = "Completed";
            if (!updated.feasibility_study_completion_date) {
              updated.feasibility_study_completion_date = today;
            }
          }
        } else if (value === "Facilities Study") {
          if (updated.feasibility_study_status !== "Completed") {
            updated.feasibility_study_status = "Completed";
            if (!updated.feasibility_study_completion_date) {
              updated.feasibility_study_completion_date = today;
          ...
          if (updated.system_impact_study_status !== "Completed") {
            updated.system_impact_study_status = "Completed";
            if (!updated.system_impact_study_completion_date) {
              updated.system_impact_study_completion_date = today;
            }
          }
        } else if (
          value === "IA Negotiation" ||
          value === "IA Executed" ||
          ...
          if (updated.facilities_study_status !== "Completed") {
            updated.facilities_study_status = "Completed";
            if (!updated.facilities_study_completion_date) {
              updated.facilities_study_completion_date = today;
            }
          }
        }
      }

      // Auto-populate security when iso_region or size_mw changes
      if (name === "iso_region" || name === "size_mw") {
        updated = autoPopulateSecurity(updated);
      }
  ...
  const autoGenerateMilestones = () => {
    const iso = form.iso_region as string;
    const sizeMw = Number(form.size_mw) || 0;
    const networkUpgrades = Number(form.grid_network_upgrades) || 0;
    const perMwRate = ISO_PER_MW_RATES[iso] || 2000;

    const today = new Date();
    const addMonths = (d: Date, months: number): string => {
      const result = new Date(d);
      result.setMonth(result.getMonth() + months);
      return result.toISOString().slice(0, 10);
    };

    const phase1Deposit = sizeMw * perMwRate;
    const phase2Deposit = networkUpgrades > 0 ? Math.round(networkUpgrades * 0.1) : sizeMw * perMwRate * 2;
    const iaSecurityAmount = Math.max(phase1Deposit, phase2Deposit) * 2;
    const constructionDeposit = networkUpgrades > 0 ? Math.round(networkUpgrades * 0.2) : iaSecurityAmount;

    const generated: MilestoneEntry[] = [
      {
        date: addMonths(today, 3),
        milestone_name: "Phase 1 Study Deposit",
        amount: phase1Deposit,
        type: "security",
        at_risk: false,
        refundable: true,
      },
      {
        ...
        milestone_name: "Phase 2 Study Deposit",
        amount: phase2Deposit,
        type: "security",
        at_risk: true,
        refundable: false,
      },
      {
        ...
        milestone_name: "IA Execution Security",
        amount: iaSecurityAmount,
        type: "security",
        at_risk: true,
        refundable: false,
      },
      {
        ...
        milestone_name: "Construction Deposit",
        amount: constructionDeposit,
        type: "security",
        at_risk: true,
        refundable: false,
      },
    ...
    setMilestones([...milestones, ...generated]);
  };
  ...
  function validateForm(): boolean {
    ...
    if (!(form.project_name as string)?.trim()) {
      errors.project_name = "Project name is required";
    }
    const sizeMw = Number(form.size_mw) || 0;
    if (sizeMw <= 0) {
      errors.size_mw = "Size (MW) must be greater than 0";
    }
    const capacityMw = Number(form.capacity_mw) || 0;
    if (capacityMw > sizeMw) {
      errors.capacity_mw = "Capacity/NRIS must be \u2264 Size (MW)";
    }
    if (!(form.iso_region as string)) {
      errors.iso_region = "ISO Region is required";
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          <InputField label="Project Name" name="project_name" value={form.project_name as string} onChange={handleChange} required error={fieldErrors.project_name} />
          ...
          <InputField label="Size (MW)" name="size_mw" type="number" value={form.size_mw as number} onChange={handleChange} required error={fieldErrors.size_mw} />
          <InputField label="Capacity/NRIS (MW)" name="capacity_mw" type="number" value={form.capacity_mw as number} onChange={handleChange} error={fieldErrors.capacity_mw} />
          ...
          <SelectField label="Project Manager" name="project_manager" value={form.project_manager as string} onChange={handleChange} options={PROJECT_MANAGERS} />
          <SelectField label="Option to Build" name="option_to_build" value={form.option_to_build as string} onChange={handleChange} options={["Self-Build", "TO-Build", "N/A"]} />
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          <SelectField label="ISO Region" name="iso_region" value={form.iso_region as string} onChange={handleChange} options={ISO_REGIONS} required error={fieldErrors.iso_region} />
          ...
          {isoRegion === "SERC" && (
            <InputField label="SERC Queue ID" name="serc_queue_id" value={form.serc_queue_id as string} onChange={handleChange} />
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          ...
          <SelectField label="Phase 1 Study Status (Feasibility)" name="feasibility_study_status" value={form.feasibility_study_status as string} onChange={handleChange} options={STUDY_STATUSES} />
          <InputField label="Phase 1 Completion Date" name="feasibility_study_completion_date" type="date" value={form.feasibility_study_completion_date as string} onChange={handleChange} />
          <SelectField label="Phase 2 Study Status (System Impact)" name="system_impact_study_status" value={form.system_impact_study_status as string} onChange={handleChange} options={STUDY_STATUSES} />
          <InputField label="Phase 2 Completion Date" name="system_impact_study_completion_date" type="date" value={form.system_impact_study_completion_date as string} onChange={handleChange} />
          ...
          <SelectField label="Phase 3 Study Status" name="phase3_study_status" value={form.phase3_study_status as string} onChange={handleChange} options={STUDY_STATUSES} />
          <InputField label="Phase 3 Completion Date" name="phase3_study_completion_date" type="date" value={form.phase3_study_completion_date as string} onChange={handleChange} />
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
        <SectionHeader title="Interconnection Results" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          ...
          <TextAreaField label="Congestion Study Results" name="congestion_results" value={form.congestion_results as string} onChange={handleChange} />
          <TextAreaField label="Power Flow Study Results" name="power_flow_results" value={form.power_flow_results as string} onChange={handleChange} />
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={autoGenerateMilestones}
